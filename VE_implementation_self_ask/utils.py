...
wiki_wiki = wikipediaapi.Wikipedia(user_agent='verify_and_edit_optimization', language= 'en')
nlp = spacy.load("en_core_web_sm")
_ENT_TYPES = ['EVENT', 'FAC', 'GPE', 'LANGUAGE', 'LAW', 'LOC', 'NORP', 'ORG', 'PERSON', 'PRODUCT', 'WORK_OF_ART']

# updated code for tokenizer
...
def get_texts_to_pages(pages, topk = 2):
    ...
    for p in pages:
        text = p.text
        texts += tokenizer.tokenize(text)[:topk]
    return texts
...
def get_texts_to_rationale_wikipedia(rationale, no_links):
    ...
    # Reduced Topk, not giving meaningful entities
    pages_to_rationale = relevant_pages_to_rationale(rationale, topk = 3)
    ...
    for i, pt in enumerate(page_titles):
        if 'disambiguation' in pt:
            # Optimized topk limit, giving ~50 pages for one entity
            new_pages += get_linked_pages([pages[i]], topk = 3)
            # new_pages += get_linked_pages([pages[i]], topk = -1)
        else:
            new_pages += [pages[i]]
    pages = new_pages
    # Omitting getting linked pages, not significant and taking lot of time.
    # if not no_links:
    #     # add linked pages
...
def read_full(args, module, slice = True,  full_num = 100):
...
def answer_again_rationale(follow_up_q, follow_up_a):
