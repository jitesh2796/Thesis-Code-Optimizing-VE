...
def consistency(answers, rationales, predictions= None):
    answer_probs = {}
    ...
    ind= [i for i in range(len(answers)) if answers[i] == final_aggregated_answer]
    best_i = max(ind, key=lambda i: len(rationales[i]))
...
def test_few_shot_manual_prediction(args):
    ...
    prompt, _ = args.helper.prompt_for_joint_prediction(dev_set[0], self_ask_prompt)
    print('prompt: ')
    print(prompt)
    if os.path.exists(result_cache_name(args)) and not args.run_length_test:
        ...
        for x in tqdm(dev_set, total=len(dev_set), desc="Predicting"):
            pred = in_context_manual_prediction(x, self_ask_prompt, engine=args.engine, prompt_helper=args.helper, length_test_only=args.run_length_test, n = args.num_shot)
            if pred != None:
