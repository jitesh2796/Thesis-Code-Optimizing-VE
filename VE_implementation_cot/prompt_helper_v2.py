...
class JointPrompHelper:
    ...
    def post_process(self, text, change_rationale=True):
        return self.post_process_prediction(text, change_rationale=change_rationale)

    def extract_answer_and_rationale(self, text):
        text = text.strip()
        sep = get_sep_text(text)
        if sep is not None:
    ...
    def post_process_prediction(self, text, change_rationale=True):
        text = text.strip()
        answer, rationale = self.extract_answer_and_rationale(text)
        result = {
            "text": text,
            "answer": answer,
        }
